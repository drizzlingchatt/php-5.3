...
RUN if [ -f /build-helpers/mysql-5.5.62-linux-glibc2.12-x86_64.tar.gz ]; then \
  cp /build-helpers/mysql-5.5.62-linux-glibc2.12-x86_64.tar.gz /tmp/; \
  else \
  wget -O /tmp/mysql-5.5.62-linux-glibc2.12-x86_64.tar.gz https://downloads.mysql.com/archives/get/p/23/file/mysql-5.5.62-linux-glibc2.12-x86_64.tar.gz; \
  fi
